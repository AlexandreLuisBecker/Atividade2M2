...
export default function CreateLivro({navigation, ...props}) {
      ...
      const onAddButtonPress = () => {
        ...
        const timeStamp = firebase.firestore.FieldValue.serverTimestamp()
          ...
          // recuperando o ID do usuário no Firebase
            ...
            const usersRef = firebase.firestore().collection('Livro')
          
            // gravando os dados do usuário criado na
            // coleção '' dentro do Firestore
            usersRef
              .doc()
              .set(data)
              ...
              .then(() => {
                ...
                // página
                navigation.navigate('Livro', { Genero: data })
              })
              .catch(error => {
                alert(error)
              })
  ...
  >
       <LinearGradient colors={['#0000CD', '#87CEFA', '#fff']}>
        ...
        <View style={styles.footerView}>
          <Text style={styles.textCadastro}>Criação Livro</Text>
        </View>
        <TextInput
          style={styles.input}
          placeholder="Nome"
